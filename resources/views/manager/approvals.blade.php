    ...
    <div class="border-b border-gray-200 mb-6">
        <nav class="-mb-px flex space-x-8">
            ...
            <button onclick="switchTab('approved')" id="approvedTab" class="border-b-2 border-transparent py-4 px-1 text-sm font-medium text-gray-500 hover:text-gray-700 hover:border-gray-300">
                Recently Approved ({{ $approvedRequests->count() }})
            </button>
        </nav>
    </div>
        ...
        @if($pendingRequests->count() > 0)
            <div class="space-y-6">
                @foreach($pendingRequests as $request)
                    <div class="bg-white shadow rounded-lg overflow-hidden hover:shadow-lg transition-shadow" id="request-card-{{ $request->id }}">
                        <!-- Request Header -->
                        <div class="px-6 py-4 bg-gradient-to-r from-indigo-50 to-white border-b border-gray-200">
                            <div class="flex items-start justify-between">
                                ...
                                <div class="text-right">
                                    <span class="px-3 py-1 text-sm font-semibold rounded-full {{ $request->request_type === 'special' ? 'bg-purple-100 text-purple-800' : 'bg-blue-100 text-blue-800' }}">
                                        {{ ucfirst($request->request_type) }}
                                    </span>
                                    <p class="text-sm font-mono font-semibold text-indigo-600 mt-2">{{ $request->sr_number }}</p>
                                    @if($request->budget_type)
                                        <span class="inline-block mt-1 px-2 py-0.5 text-xs font-semibold rounded-full {{ $request->budget_type === 'budgeted' ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800' }}">
                        ...
                        <div class="px-6 py-4 space-y-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-900 mt-1">{{ $request->purpose }}</p>
                            </div>
                            ...
                            @if($request->request_type === 'standard')
                                <div>
                                    <div class="flex items-center justify-between mb-2">
                                        <p class="text-sm font-medium text-gray-700">Items ({{ $request->items->count() }}):</p>
                                        <button onclick="toggleEditMode({{ $request->id }})" 
                                        ...
                                        <table class="min-w-full">
                                            <thead>
                                                <tr class="border-b border-gray-200 bg-gray-100">
                                                    <th class="text-left text-xs font-semibold text-gray-600 uppercase px-4 py-2">Item</th>
                                                    <th class="text-left text-xs font-semibold text-gray-600 uppercase px-4 py-2">Code</th>
                                                    <th class="text-center text-xs font-semibold text-gray-600 uppercase px-4 py-2">Quantity</th>
                                                    <th class="text-left text-xs font-semibold text-gray-600 uppercase px-4 py-2">Unit</th>
                                                    <th class="text-center text-xs font-semibold text-gray-600 uppercase px-4 py-2 edit-header-{{ $request->id }} hidden">Action</th>
                                                </tr>
                                            </thead>
                                            <tbody class="divide-y divide-gray-200">
                                                @foreach($request->items as $item)
                                                    <tr class="hover:bg-gray-100 transition">
                                                        ...
                                                        <td class="px-4 py-2 text-sm text-gray-500 font-mono">{{ $item->item_code }}</td>
                                                        <td class="px-4 py-2 text-center">
                                                            <span class="qty-display-{{ $item->id }} text-sm font-semibold text-gray-900">{{ $item->quantity }}</span>
                                                            <input type="number" 
                                                                   class="qty-edit-{{ $item->id }} hidden w-20 mx-auto text-center rounded-md border-gray-300 text-sm" 
                                                                   value="{{ $item->quantity }}" 
                                                                   min="1" max="9999">
                                                        </td>
                                                        <td class="px-4 py-2 text-sm text-gray-500">{{ $item->supply->unit ?? 'pcs' }}</td>
                                                        <td class="px-4 py-2 text-center edit-actions-{{ $request->id }} hidden">
                                                            <div class="flex items-center justify-center gap-2">
                                                                <button onclick="saveItemQty({{ $request->id }}, {{ $item->id }})" 
                                                                        class="text-green-600 hover:text-green-800 text-xs font-medium">
                                                                    Save
                                                                </button>
                                                                <button onclick="cancelItemEdit({{ $item->id }})" 
                        ...
                        <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex flex-col sm:flex-row justify-end gap-3">
                            <a href="{{ route('manager.requests.show', $request->id) }}" 
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase">Budget</th>
                        ...
                        @foreach($approvedRequests as $request)
                            ...
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-3 whitespace-nowrap">
                                    <div class="text-sm font-mono text-indigo-600 font-semibold">{{ $request->sr_number }}</div>
                                    @if($request->serial_number)
                                        <div class="text-xs font-mono text-gray-400">{{ $request->serial_number }}</div>
                                    @endif
                                </td>
                                <td class="px-4 py-3 whitespace-nowrap">
                                ...
                                <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500">
                                    {{ $request->manager_approved_at->format('M d, Y') }}
                                    <div class="text-xs text-gray-400">{{ $request->manager_approved_at->format('h:i A') }}</div>
                                </td>
                                <td class="px-4 py-3 whitespace-nowrap text-center">
                                    @if($isFullyReleased)
...
function toggleEditMode(requestId) {
    // Toggle edit header
    document.querySelectorAll(`.edit-header-${requestId}`).forEach(el => el.classList.toggle('hidden'));
    
    // Toggle action cells
    document.querySelectorAll(`.edit-actions-${requestId}`).forEach(el => el.classList.toggle('hidden'));
    
    // Get card items
    const card = document.getElementById(`request-card-${requestId}`);
    const items = card.querySelectorAll('[class^="qty-display-"]');
    
    items.forEach(display => {
        const itemId = display.className.match(/qty-display-(\d+)/)[1];
        const edit = card.querySelector(`.qty-edit-${itemId}`);
...
function cancelItemEdit(itemId) {
    const display = document.querySelector(`.qty-display-${itemId}`);
    const edit = document.querySelector(`.qty-edit-${itemId}`);
    
    // Reset to original value
    edit.value = display.textContent.trim();
}

async function saveItemQty(requestId, itemId) {
    const input = document.querySelector(`.qty-edit-${itemId}`);
    ...
    try {
        const res = await fetch(`/manager/requests/${requestId}/update-item`, {
            method: 'POST',
            headers: {
        ...
        if (data.success) {
            // Update display
            document.querySelector(`.qty-display-${itemId}`).textContent = newQty;
