...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="mb-6">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('employee.requests.index') }}" class="text-sm text-indigo-600 hover:text-indigo-900">
                ← Back to Requests
            </a>
        </div>
    </div>
        ...
        <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-gray-600">Submitted {{ $request->created_at->format('F d, Y') }}</p>
                </div>
        ...
        <div class="p-6 space-y-6">
                ...
                <div class="bg-indigo-50 border-l-4 border-indigo-600 p-4">
                    <div class="flex items-center">
                        ...
                        <div>
                            ...
                            <p class="text-xl font-mono font-bold text-indigo-900">{{ $request->serial_number }}</p>
                        </div>
                    </div>
                </div>
            @endif
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <span class="px-3 py-1 text-sm font-semibold rounded {{ $request->request_type === 'special' ? 'bg-purple-100 text-purple-800' : 'bg-blue-100 text-blue-800' }}">
                        {{ ucfirst($request->request_type) }}
                    </span>
                </div>
                <div>
                    <h3 class="text-sm font-medium text-gray-700 mb-2">Department</h3>
                    <p class="text-sm text-gray-900">{{ $request->department->name }} ({{ $request->department->code }})</p>
                </div>
            </div>
            ...
            @if($request->request_type === 'standard')
                <div>
                    <h3 class="text-sm font-medium text-gray-700 mb-3">Requested Items ({{ $request->items->count() }})</h3>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($request->items as $item)
                                    <tr>
                                        ...
                                        <td class="px-4 py-3 text-sm text-gray-900 text-right font-semibold">{{ $item->quantity }}</td>
                                        <td class="px-4 py-3 text-sm text-gray-500">{{ $item->supply->unit ?? 'N/A' }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                                ...
                                <div class="relative flex space-x-3">
                                    ...
                                    <div class="min-w-0 flex-1 pt-1.5 flex justify-between space-x-4">
                                        <div>
                                            ...
                                            <p class="text-xs text-gray-500">By {{ $request->user->name }}</p>
                                        </div>
                                    ...
                                    <div class="relative flex space-x-3">
                                        ...
                                        <div class="min-w-0 flex-1 pt-1.5 flex justify-between space-x-4">
                                            <div>
                                                ...
                                                <p class="text-xs text-gray-500">By {{ $request->managerApprover->name ?? 'N/A' }}</p>
                                                @if($request->manager_notes)
                                                    <p class="text-xs text-gray-600 mt-1 italic">"{{ $request->manager_notes }}"</p>
                                                @endif
                                            </div>
                                            <div class="text-right text-sm whitespace-nowrap text-gray-500">
                                                {{ $request->manager_approved_at->format('M d, Y h:i A') }}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="relative flex space-x-3">
                                        ...
                                        <div class="min-w-0 flex-1 pt-1.5 flex justify-between space-x-4">
                                            <div>
                                                ...
                                                <p class="text-xs text-gray-500">By {{ $request->managerApprover->name ?? 'N/A' }}</p>
                                                @if($request->manager_notes)
                                                    <p class="text-xs text-red-600 mt-1 italic">"{{ $request->manager_notes }}"</p>
                                                @endif
                                            </div>
                                            <div class="text-right text-sm whitespace-nowrap text-gray-500">
                                                {{ $request->manager_rejected_at->format('M d, Y h:i A') }}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="relative flex space-x-3">
                                        ...
                                        <div class="min-w-0 flex-1 pt-1.5 flex justify-between space-x-4">
                                            <div>
                                                ...
                                                <p class="text-xs text-gray-500">By {{ $request->adminReleaser->name ?? 'N/A' }}</p>
                                                @if($request->admin_notes)
                                                    <p class="text-xs text-gray-600 mt-1 italic">"{{ $request->admin_notes }}"</p>
                                                @endif
                                            </div>
                                            <div class="text-right text-sm whitespace-nowrap text-gray-500">
                                                {{ $request->admin_released_at->format('M d, Y h:i A') }}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="relative flex space-x-3">
                                        ...
                                        <div class="min-w-0 flex-1 pt-1.5 flex justify-between space-x-4">
                                            <div>
                                                ...
                                                <p class="text-xs text-gray-500">By {{ $request->adminReleaser->name ?? 'N/A' }}</p>
                                                @if($request->admin_notes)
                                                    <p class="text-xs text-red-600 mt-1 italic">"{{ $request->admin_notes }}"</p>
                                                @endif
                                            </div>
                                            <div class="text-right text-sm whitespace-nowrap text-gray-500">
                                                {{ $request->admin_rejected_at->format('M d, Y h:i A') }}
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
async function cancelRequest() {
    ...
    try {
        const response = await fetch('/employee/requests/{{ $request->id }}', {
            method: 'DELETE',
            headers: {
