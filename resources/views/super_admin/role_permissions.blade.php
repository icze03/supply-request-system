...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <!-- Permission Form -->
    <form method="POST" action="{{ route('super_admin.role_permissions.update') }}" id="permForm">
        @csrf
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div>
                    <h2 class="text-lg font-semibold text-gray-900">
                        ...
                        <span class="ml-2 px-3 py-1 rounded-full text-sm font-bold
                            {{ $selectedRole === 'admin' ? 'bg-red-100 text-red-700'
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    ...
                    @php $checked = in_array($perm->id, $assignedIds); @endphp
                    <label class="perm-card flex items-start gap-3 p-4 rounded-xl border-2 cursor-pointer transition
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-semibold text-gray-800">{{ $perm->name }}</p>
                            <p class="text-xs text-gray-500 mt-0.5 truncate font-mono">{{ $perm->url_prefix ?? '/' }}</p>
                        </div>
            ...
            <div class="px-6 py-4 bg-gray-50 border-t border-gray-100 flex items-center justify-between">
                <p class="text-xs text-gray-500">
                    <span id="checkedCount">{{ count($assignedIds) }}</span> of {{ $permissions->count() }} permissions enabled
                </p>
                <div class="flex items-center gap-3">
