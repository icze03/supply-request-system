...
<div class="screen-wrapper">
    ...
    <div class="no-print">
        ...
        <a class="btn-back" href="{{ route('admin.releases.index') }}">← Back to Releases</a>
    </div>
            ...
            <div class="meta-grid">
                <div>
                    <div class="meta-field">
                        ...
                        <span class="meta-value">{{ $request->department->name }}</span>
                    </div>
                    <div class="meta-field">
                        <span class="meta-label">Cost Center:</span>
                        <span class="meta-value">{{ $request->department->cost_center ?? $request->department->code }}</span>
                    </div>
                </div>
                <div>
                    <div class="meta-field">
                        <span class="meta-label">Control No.:</span>
                        <span class="meta-value" style="font-family:monospace;">{{ $request->serial_number }}</span>
                    </div>
                    @if($request->ro_number)
                    <div class="meta-field">
                        <span class="meta-label">RO Number:</span>
                        <span class="meta-value" style="font-family:monospace;">{{ $request->ro_number }}</span>
                    </div>
                    @endif
                    @if($totalPages > 1)
                    <div class="meta-field">
                        <span class="meta-label">Page:</span>
                        <span class="meta-value">{{ $page + 1 }} of {{ $totalPages }}</span>
                    </div>
                    @endif
                </div>
            </div>

            {{-- Table wrapper (stretches between meta and signatures) --}}
                    ...
                    <tbody>
                        ...
                        @if ($request->request_type === 'standard')
                            ...
                            @foreach ($pageItems as $item)
                                ...
                                @endphp
                                <tr class="data-row">
                                    <td class="text-center" style="font-family:monospace;font-size:8px;">{{ $item->item_code }}</td>
                                    <td>{{ $item->item_name }}</td>
                                    <td class="text-center">{{ $item->quantity }}</td>
                                    <td class="text-center {{ $isPartial ? 'partial-cell' : '' }}">{{ $allocated }}</td>
                                    <td class="text-center">{{ $item->supply->unit ?? 'pcs' }}</td>
                                    <td class="text-right">{{ $unitCost !== null ? '₱' . number_format($unitCost, 2) : '' }}</td>
                                    <td class="text-right">{{ $lineTotal !== null ? '₱' . number_format($lineTotal, 2) : '' }}</td>
                                </tr>
                            @endforeach

                            {{-- Single filler row — stretches to fill all remaining height --}}
                            <tr class="filler-row">
                        ...
                        @else
                            ...
                            @if ($page === 0)
                                <tr class="data-row">
                                    ...
                                    <td>{{ $request->special_item_description }}</td>
            ...
            <div class="signature-grid">
                <div>
                    <div class="sig-block">
                        ...
                        <div class="sig-line">{{ $request->created_at->format('M d, Y  h:i A') }}</div>
                    </div>
                    <div class="sig-block">
                        <span class="sig-label">Approved by:</span>
                        <div class="sig-line">{{ $request->managerApprover->name ?? '' }}</div>
                        <span class="sig-date-label">Date:</span>
                        <div class="sig-line">{{ $request->manager_approved_at ? $request->manager_approved_at->format('M d, Y  h:i A') : '' }}</div>
                    </div>
                </div>
                <div>
                    <div class="sig-block">
                        <span class="sig-label">Issued by:</span>
                        <div class="sig-line">{{ $request->adminReleaser->name ?? '' }}</div>
                        <span class="sig-date-label">Date:</span>
                        <div class="sig-line">{{ $request->admin_released_at ? $request->admin_released_at->format('M d, Y  h:i A') : '' }}</div>
                    </div>
                    <div class="sig-block">
