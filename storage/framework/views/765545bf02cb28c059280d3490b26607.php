...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="bg-white shadow rounded-lg p-6 mb-6">
        <form method="GET" action="<?php echo e(route('employee.catalog')); ?>" id="filterForm">
                ...
                <div class="flex items-end">
                    <div class="flex items-center gap-3 w-full">
                        ...
                        <?php if(request('search') || request('category')): ?>
                            <a href="<?php echo e(route('employee.catalog')); ?>" class="px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 hover:bg-gray-50">
                                Clear Filters
                            </a>
                        <?php endif; ?>
                    </div>
                ...
                <?php $__empty_1 = true; $__currentLoopData = $supplies; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $supply): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            <?php if($supply->unit_cost): ?>
                                <span class="text-sm font-semibold text-gray-800">₱<?php echo e(number_format($supply->unit_cost, 2)); ?></span>
                            <?php else: ?>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            <div class="flex items-center justify-center border border-gray-200 rounded-lg overflow-hidden w-28 mx-auto">
                                <button type="button" onclick="document.getElementById('qty-<?php echo e($supply->id); ?>').stepDown()"
                                ...
                                <button type="button" onclick="document.getElementById('qty-<?php echo e($supply->id); ?>').stepUp()"
                                    class="px-2.5 py-1.5 text-gray-500 hover:bg-gray-100 text-sm font-bold transition">+</button>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            <button onclick="addToCart(<?php echo e($supply->id); ?>, '<?php echo e(addslashes($supply->name)); ?>', '<?php echo e($supply->item_code); ?>', '<?php echo e($supply->unit); ?>')"
...
document.getElementById('specialRequestForm')?.addEventListener('submit', async function(e) {
    ...
    try {
        const response = await fetch('<?php echo e(route("employee.requests.special")); ?>', {
            method: 'POST',
        ...
        if (data.success) {
            ...
            setTimeout(() => { window.location.href = '<?php echo e(route("employee.requests.index")); ?>'; }, 1000);
...
async function submitRequest() {
    ...
    try {
        const response = await fetch('<?php echo e(route("employee.requests.store")); ?>', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content },
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\supply-request-system\resources\views/employee/catalog.blade.php ENDPATH**/ ?>